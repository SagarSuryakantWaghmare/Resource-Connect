...
import { FaUserCircle, FaWindowClose } from 'react-icons/fa'; // Icon for user
                    ...
                    {jobPosts.length > 0 ? (
                        jobPosts.map((job) => (
                            <div key={job._id} className="bg-white p-4 rounded-lg shadow flex items-start">
                                ...
                                <div className="ml-4 flex flex-col-reverse gap-4 items-center justify-center">
                                    <Link to={`/job/${job._id}`} className="text-stdYellow font-bold">
                                        View Details
                                    </Link>
                                    <div className="ml-0 flex flex-col space-y-2">
                                        <button className="px-4 py-2 bg-green-500 text-white rounded-full">Accept</button>
                                        <button className="px-4 py-2 bg-red-500 text-white rounded-full">Reject</button>
                                    </div>
                                </div>
                            </div>
                        ))
